...
Local source and installed behavior outrank these examples. A real project's checkout already installed Vitest; its `--reporter=json --outputFile=...` run was checked on one representative test file before integration.
...
A local coding-agent CLI that supports tool-less, session-less prompt execution is a possible reviewer route, not an implemented or validated reviewer. A human-labelled golden set and an independent rating check are required before any trusted numeric score.
