...
const unjudged = {
  design: 'Check with AI, or compare the page with the rest of your app.',
  purpose: 'Check with AI, or say what this page is for.',
  ease: 'Check with AI, or try the page on a phone and a computer.',
};
const noScreenshots = 'Take screenshots first; then the AI or you can answer.';
...
function unjudgedText(page, key) {
  return page.captures.desktop.state === 'rendered' ? unjudged[key] : noScreenshots;
}

function aiPart(page, key, ai) {
  const judgment = ai?.dimensions?.[key];
  if (!judgment) return part('untested', unjudgedText(page, key));
...
function judgedPart(page, key, ai) {
  ...
  if (verdict.status === 'untested') return aiPart(page, key, ai);
