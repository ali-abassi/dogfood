...
// quarter and leaves the rest blank. Real pages almost never leave this share blank.
...
export function captureProblem(bytes) {
  ...
  const from = Math.floor(image.width * (1 - blankShare));
  if (!regionIsBlank(image, from)) return '';
  return `The right ${Math.round(blankShare * 100)}% of the screenshot is blank, so the page did not fill the image. This usually means the wrong device pixel ratio; capture the page again.`;
}

function regionIsBlank(image, from) {
  const { width, height, bpp, pixels } = image;
  const reference = pixels.subarray((width - 1) * bpp, width * bpp);
