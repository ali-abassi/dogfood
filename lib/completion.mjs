...
const requirements = [
  ...
  {
    id: 'ai-review',
    label: 'AI review of the current screenshots',
    met: (page, evidence) => evidence.visualReview.exists && !evidence.visualReview.stale,
    missing: (page, evidence) => evidence.visualReview.exists ? 'The AI review belongs to older screenshots; run it again.' : 'Ask AI to review the screenshots.',
  },
  {
    id: 'issues',
...
// True when the page was reviewed and a later scan found it looks different, so the
// verdicts describe a page that no longer exists. Informational: it does not block completion.
export function changedSinceReview(page) {
  const changed = devices.some(device => page.scan?.changes?.[device]?.changed);
  if (!changed || !verdictEntries(page).some(reviewed)) return false;
  return latestVerdictTime(page) < Date.parse(page.scan.scannedAt);
}
