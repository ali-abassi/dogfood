...
import { plainFailure, scanPages, withScanner } from './scanner.mjs';
...
// The first page is where an unreachable address shows up, so its failure is put in plain words.
async function findPages(browser, url) {
  try {
    return await discoverPages(browser, url);
  } catch (error) {
    throw new Error(plainFailure(error.message, 'that address'));
  }
}

export function onboard(plan, onProgress = () => {}) {
  return withScanner(plan.browserProfile, async browser => {
    const pages = await findPages(browser, plan.url);
