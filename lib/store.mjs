...
import { pageCompletion, pageStatus } from './completion.mjs';
import { capturesDir, dataDir, projectsDir, root } from './paths.mjs';
import {
  auditKeys, captureTiers, checkKeys, connectionProvenance, evidenceNote, findingIdPattern,
  httpMethods, idPattern, rowIdPattern, severities, testFilePattern, verdicts,
} from './schema.mjs';
...
export function pageProgress(project, page) {
  const { review, stale } = latestVisualReview(dataDir, project.id, page.id, page.capture);
  const completion = pageCompletion(page, { visualReview: { exists: Boolean(review), stale } });
  return { status: pageStatus(page, completion), ...completion };
}
...
export function createProject(input) {
  const id = text(input?.id, 'Project ID', 1, 80);
  ...
  return writeProject({
    ...
    description: text(input.description, 'Project description', 1, 400),
    source: { url: httpUrl(input.url, 'Product URL'), environment: text(input.environment, 'Environment', 1, 200), checkout: text(input.checkout, 'Local checkout', 1, 500) },
...
function registeredFeatures(input, current) {
  return uniqueRows(input, 'Features', 1, 60).map(row => {
    const previous = current.find(item => item.id === row.id) ?? { status: 'untested', note: '' };
    return { ...previous, id: row.id, name: text(row.name, 'Feature name', 1, 120) };
  });
}
...
function newPage(id) {
  return {
    id,
    capture: { state: 'blocked', reason: 'Not captured yet.' },
    features: [],
    checks: Object.fromEntries(checkKeys.map(key => [key, { status: 'untested', note: '' }])),
    findings: [],
    audit: Object.fromEntries(auditKeys.map(key => [key, defaultAudit[key].map(([rowId, question]) => ({ id: rowId, question, status: 'untested', note: '' }))])),
    connections: [],
    qa: { tests: [], note: '' },
  };
}
...
function registeredPlan(project, page, input) {
  ...
  return { ...page.qa, tests, note: text(input.untestedNote, 'Untested boundary note', 12, 600) };
}
...
function storeCaptureFile(projectId, pageId, source) {
  const directory = join(capturesDir, projectId);
  const target = join(directory, `${pageId}.png`);
  mkdirSync(join(directory, 'history'), { recursive: true });
  if (existsSync(target)) renameSync(target, join(directory, 'history', `${pageId}-${Date.now()}.png`));
  copyFileSync(source, target);
  return `/captures/${projectId}/${pageId}.png`;
}
...
function renderedCapture(projectId, pageId, input) {
  ...
  const path = storeCaptureFile(projectId, pageId, file);
...
export function recordCapture(projectId, pageId, input) {
  ...
  page.capture = input?.blockedReason ? blockedCapture(input) : renderedCapture(projectId, pageId, input ?? {});
  return writeProject(project);
}
